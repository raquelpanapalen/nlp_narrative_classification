...
# https://github.com/saadnaeem-dev/pytorch-linear-warmup-cosine-annealing-warm-restarts-weight-decay/blob/main/linear_warmup_cosine_annealing_warm_restarts_weight_decay/lr_scheduler.py
...
class CosineAnealingWarmRestartsWeightDecay(_LRScheduler):
    ...
    def __init__(
        ...
        super(CosineAnealingWarmRestartsWeightDecay, self).__init__(
    ...
    def step(self, epoch=None):
        ...
        if self.epoch >= self.T_0:
            if self.T_mul == 1.0:
                ...
                n = int(
                    math.log((self.epoch / self.T_0 * (self.T_mul - 1) + 1), self.T_mul)
                )
                self.cycle = n
                self.T_cur = self.epoch - int(
                    self.T_0 * (self.T_mul**n - 1) / (self.T_mul - 1)
                )
...
class ChainedScheduler(_LRScheduler):
    ...
    def __init__(
        ...
        if warmup_steps != 0:
            ...
            warmup_steps = (
                warmup_steps + 1
            )  # directly refers to epoch account for 0 off set
