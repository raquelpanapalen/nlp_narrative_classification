...
def baseline(args):
    ...
    def construct_dataloader_bert(split, topic):
        data_paths = {
            "main": args.data_path,
            "additional": args.additional_data,
        }
        dataset = BERTDeepLNarrativeDataset(data_paths, topic, split, val_split=args.val_split)
        return DataLoader(dataset, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True)
    ...
    for topic in prediction_topic:
        ...
        elif (args.model in deep_learning_models) or (args.model == "bert"):
            loaders = {}
        ...
        if args.model in traditional_models:
            ...
            model, tokenizer = get_model(args.model, num_classes=len(index2label))
            model.to(device)

            def infer_with_bert(model, tokenizer, text, device):
            ...
            loaders = {}
            for split in ["val"]:
                loaders[split] = construct_dataloader_bert(split, topic)
            ...
            for split in ["train", "val", "dev"]:  
                for batch in loaders[split]:
                    ...
                    # predictions_batch = infer_with_bert(model, tokenizer, texts, device)
                    ...
                    y_pred.extend(predictions_batch.numpy())
                    

                    for i, doc_name in enumerate(batch['input_ids']):
                        y_true.append(batch['labels'][i].tolist())  
                        y_pred.append(index2label[predictions_batch[i].item()]) 
                        predictions[doc_name] = {"labels": [index2label[predictions_batch[i].item()]]}
